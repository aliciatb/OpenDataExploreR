...
selectInput("domain", label = "State Domain:",
            choices = list(
                        ...
                        `Michigan` = "data.michigan.gov",
                         # something weird with school results and need to clean up financial datasets
                         #`MI Community Financial Dashboard` = 'mi-treasury.data.socrata.com',
                         #`MI School Financial Data` = 'mischooldata.data.socrata.com',
                        `New Jersey` = "data.nj.gov",
                        `New York` = "data.ny.gov",
                        `New York Department of Health` = "healthdata.ny.gov",
...
selectInput("asset_type", label = "Type:",
            choices = c("calendar","chart","datalens","dataset","federated_href","file","filter","form","href","link","map","story","visualization"), selected="dataset")
...
renderPlot({
  ...
  coord_flip() +
  labs(x = '', y = '', title=paste0('Top ',input$n_datasets,' ',input$asset_type,'s for ',input$domain,' by ',str_to_title(str_replace_all(input$rank,'_',' ')))) +
...
renderPlot({
  dataset_descriptions <- discoData() %>%
